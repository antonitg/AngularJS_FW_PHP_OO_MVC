...
class logreg_dao
{
    ...
    public function setRecoverToken($token,$email)
    {
       $typedQuery = "UPDATE users SET tokenRecover = '{$token}' WHERE email = '{$email}'";
        return db::query()->manual($typedQuery)
            ->execute()
            ->toJSON();
    }
    public function recoverToken($token,$passwd)
    {
       $typedQuery = "UPDATE users SET tokenRecover = NULL, passwd = '{$passwd}' WHERE tokenRecover = '{$token}'";
        return db::query()->manual($typedQuery)
            ->execute()
            ->toJSON();
    }
	...
	public function social($pass,$fullname,$username,$email,$profilepic){
		       $typedQuery = "INSERT INTO `users` (`fullname`, `username`, `email`, `passwd`, `avatar`) VALUES ('{$fullname}', '{$username}', '{$email}', '{$pass}', '{$profilepic}')";
