...
class logreg_bll
{
	...
	public function social($args) {
		// secret , uid , username , email , photo
		$this->dao->register("Social Signin", $args[2], $args[3], $args[3], $args[4]);
		$token = jwt_process::encode($args[0], $args[3]);
		$total = array("message" => 'Succefully loged in!', "token" => $token, "page" => '#shop');
    ...
    public function recoverToken($args) {
        $pwd_peppered = hash_hmac("sha256", $args[1], $args[2]);
        $pwd_hashed = password_hash($pwd_peppered, PASSWORD_DEFAULT);
        return $this -> dao -> recoverToken($args[0],$pwd_hashed);
        // token passwd secret
    }
    ...
    public function setRecoverToken($args) {
        return $this -> dao -> setRecoverToken($args[0],$args[1]);
    }
}