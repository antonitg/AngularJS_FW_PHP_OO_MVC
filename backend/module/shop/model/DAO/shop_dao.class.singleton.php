...
class shop_dao
{
    ...
    public function getFiltersSearch($keyword, $brand, $condition, $minprice, $maxprice, $showing, $page, $user)
    {
        ...
        $typedQuery = "SELECT c.*,v.registrationfav,i.src FROM `cars` c LEFT JOIN `favs` v ON v.userfav = '{$user}' AND v.registrationfav=c.registration LEFT JOIN `img` i ON i.id=c.registration WHERE `brand` LIKE '{$brand}' AND `carcondition` LIKE '{$condition}' AND `model` LIKE '%{$keyword}%' AND `price` BETWEEN '{$minprice}' AND '{$maxprice}' ORDER BY views desc LIMIT {$from},{$showing}";
    ...
    {
        $typedQuery = "DELETE FROM cart WHERE cartuser='$user'";
        return db::query()->manual($typedQuery)
            ->execute()
            ->queryToArray()
            ->toJSON();
